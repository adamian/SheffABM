...
def connectPorts():
    if not( yarp.Network.connect("/icubSim/cam/left", "/interface/cam/left") ):
        return False

    if not( yarp.Network.connect("/icubSim/cam/right", "/interface/cam/right") ):
        return False
...
def disconnectPorts():
    yarp.Network.disconnect("/icubSim/cam/left", "/interface/cam/left")
    yarp.Network.disconnect("/icubSim/cam/right", "/interface/cam/right")
    return True
...
#    filename = '../database/left_cam/png/left_cam_' + str(imageCounter) + '.png'
    filename = '../database/left_cam/png/left_cam.png'
...
#    Image.open('../database/left_cam/png/left_cam_' + str(imageCounter) + '.png').save('../database/left_cam/jpg/left_cam_' + str(imageCounter) + '.jpg', 'JPEG')
    Image.open('../database/left_cam/png/left_cam.png').save('../database/left_cam/jpg/left_cam.jpg', 'JPEG')
...
#    filename = '../database/right_cam/png/right_cam_' + str(imageCounter) + '.png'
    filename = '../database/right_cam/png/right_cam.png'
    plt.savefig(filename,bbox_inches='tight', pad_inches=0, dpi=50)
#    Image.open('../database/right_cam/png/right_cam_' + str(imageCounter) + '.png').save('../database/right_cam/jpg/right_cam_' + str(imageCounter) + '.jpg', 'JPEG')
    Image.open('../database/right_cam/png/right_cam.png').save('../database/right_cam/jpg/right_cam.jpg', 'JPEG')
    camerasInfoBottle.addString(filename)
